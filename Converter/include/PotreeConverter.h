...
inline ScaleOffset computeScaleOffset(Vector3 min, Vector3 max,
                                      Vector3 targetScale) {
  ...
  // since the Potree 1.7 release mistakenly interprets the coordinates as uint
  // values, we can't do that and we use 0/0/0 as the bounding box minimum as
  // the origin instead.
...
inline vector<Attribute> computeOutputAttributes(LasHeader &header) {
  ...
  Attribute wavePacketDescriptorIndex("wave packet descriptor index", 1, 1, 1,
                                      AttributeType::UINT8);
  Attribute byteOffsetToWaveformData("byte offset to waveform data", 8, 1, 8,
                                     AttributeType::UINT64);
  Attribute waveformPacketSize("waveform packet size", 4, 1, 4,
                               AttributeType::UINT32);
  Attribute returnPointWaveformLocation("return point waveform location", 4, 1,
                                        4, AttributeType::FLOAT);
  Attribute XYZt("XYZ(t)", 12, 3, 4, AttributeType::FLOAT);
  Attribute classificationFlags("classification flags", 1, 1, 1,
                                AttributeType::UINT8);
  ...
  if (format == 0) {
    list = {
        xyz,           intensity, returnNumber, numberOfReturns, classification,
...
inline Attributes computeOutputAttributes(vector<Source> &sources,
                                          vector<string> requestedAttributes) {
  ...
  // compute scale and offset from all sources
  {
    ...
    for_each(sources.begin(), sources.end(),
             [&mtx, &sources, &scaleMin, &min, &max, requestedAttributes,
              &fullAttributeList, &acceptedAttributeNames](Source source) {
#else
             ...
             [&mtx, &sources, &scaleMin, &min, &max, requestedAttributes,
              &fullAttributeList, &acceptedAttributeNames](Source source) {
               ...
               for (auto &attribute : attributes) {
                 bool alreadyAdded =
                     acceptedAttributeNames.find(attribute.name) !=
                     acceptedAttributeNames.end();
    ...
    if (scaleMin.x != scale.x || scaleMin.y != scale.y ||
        scaleMin.z != scale.z) {
  ...
  if (requestedAttributes.size() > 0) {
    ...
    for (string name : should) {
      auto it = find_if(is.begin(), is.end(),
  ...
  // 	cout << rightPad("name", c0) << leftPad("offset", c1) << leftPad("size",
  // c2) << endl; 	cout << string(ct, '=') << endl;
...
inline string toString(Attributes &attributes) {
  ...
  ss << rightPad("name", c0) << leftPad("offset", c1) << leftPad("size", c2)
     << endl;
  ss << string(ct, '=') << endl;

  int offset = 0;
  for (auto attribute : attributes.list) {
    ss << rightPad(attribute.name, c0) << leftPad(formatNumber(offset), c1)
       << leftPad(formatNumber(attribute.size), c2) << endl;
