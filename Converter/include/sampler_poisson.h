...
struct SamplerPoisson : public Sampler {
  ...
  void sample(Node *node, Attributes attributes, double baseSpacing,
    ...
    function<void(Node *, function<void(Node *)>)> traversePost =
        [&traversePost](Node *node, function<void(Node *)> callback) {
    ...
    traversePost(node, [bytesPerPoint, baseSpacing, scale, offset,
                        &onNodeCompleted, &onNodeDiscarded,
      ...
      for (int64_t childIndex = 0; childIndex < 8; childIndex++) {
        ...
        for (int64_t i = 0; i < child->numPoints; i++) {
          ...
          int32_t *xyz =
              reinterpret_cast<int32_t *>(child->points->data_u8 + pointOffset);
      ...
      auto checkAccept = [/*&dbgChecks, &dbgSumChecks,*/ &dbgNumAccepted,
                          spacing, squaredSpacing, &squaredDistance,
                          center /*, &numDistanceChecks*/](Point candidate) {
        ...
        for (int64_t i = dbgNumAccepted - 1; i >= 0; i--) {
          ...
          // stop when differences to center between candidate and accepted
          // exceeds the spacing any other previously accepted point will be
      ...
      for (Point point : points) {
        ...
        //			debug += 0.01;
        //			memcpy(data + point.pointIndex *
        //attributes.bytes + 20, &debug, 8);
        //		//}
        //		//double value = now();
        ...
        acceptedChildPointFlags[point.childIndex][point.pointIndex] =
            isAccepted ? 1 : 0;
      ...
      for (int64_t childIndex = 0; childIndex < 8; childIndex++) {
        ...
        for (int64_t i = 0; i < child->numPoints; i++) {
          ...
          if (isAccepted) {
            accepted->write(child->points->data_u8 + pointOffset,
                            attributes.bytes);
            // rejected->write(child->points->data_u8 + pointOffset,
            // attributes.bytes);
          } else {
            rejected->write(child->points->data_u8 + pointOffset,
                            attributes.bytes);
      ...
      //	string msg = "#checks: " + formatNumber(dbgSumChecks) + ",
      //maxChecks: " + formatNumber(dbgMaxChecks) + ", avgChecks: " +
      //formatNumber(avgChecks) + "\n"; 	cout << msg;
      //}
