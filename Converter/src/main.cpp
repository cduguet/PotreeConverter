...
Options parseArguments(int argc, char **argv) {
  ...
  args.addArgument("projection",
                   "Add the projection of the pointcloud to the metadata");
  args.addArgument("generate-page,p",
  ...
  if (args.has("outdir")) {
    ...
    while (fs::exists(outdir)) {
      outdir =
          sourcepath + "/../" + suggestedBaseName + "_" + std::to_string(i);

      if (i > 100) {

        logger::ERROR("unsuccessfully tried to find empty output directory. "
                      "stopped at 100 iterations: " +
                      outdir);
...
Stats computeStats(vector<Source> sources) {
  ...
  string strMin = "[" + to_string(min.x) + ", " + to_string(min.y) + ", " +
                  to_string(min.z) + "]";
  string strMax = "[" + to_string(max.x) + ", " + to_string(max.y) + ", " +
                  to_string(max.z) + "]";
  string strSize = "[" + to_string(size.x) + ", " + to_string(size.y) + ", " +
                   to_string(size.z) + "]";
  ...
  string strTotalFileSize;
  {
    ...
    if (totalBytes >= TB) {
      strTotalFileSize =
          formatNumber(double(totalBytes) / double(TB), 1) + " TB";
    } else if (totalBytes >= GB) {
      strTotalFileSize =
          formatNumber(double(totalBytes) / double(GB), 1) + " GB";
    } else if (totalBytes >= MB) {
      strTotalFileSize =
          formatNumber(double(totalBytes) / double(MB), 1) + " MB";
    } else {
...
// 			double throughput = (double(state.pointsProcessed) /
// state.duration) / 1'000'000.0;

// 			double progressPass = 100.0 * state.progress();
// 			double progressTotal = (100.0 * double(state.currentPass
// - 1) + progressPass) / double(state.numPasses);

// 			string strProgressPass = formatNumber(progressPass) +
// "%"; 			string strProgressTotal = formatNumber(progressTotal) + "%"; 			string
// strTime = formatNumber(now()) + "s"; 			string strDuration =
// formatNumber(state.duration) + "s"; 			string strThroughput =
// formatNumber(throughput) + "MPs";

// 			string strRAM =
// formatNumber(double(ram.virtual_usedByProcess) / GB, 1)
// 				+ "GB (highest " +
// formatNumber(double(ram.virtual_usedByProcess_max) / GB, 1) + "GB)"; 			string
// strCPU = formatNumber(CPU.usage) + "%";

// 			stringstream ss;
// 			ss << "[" << strProgressTotal << ", " << strTime << "],
// "
// 				<< "[" << state.name << ": " << strProgressPass
// << ", duration: " << strDuration << ", throughput: " << strThroughput << "]"
...
void chunking(Options &options, vector<Source> &sources, string targetDir,
              Stats &stats, State &state, Attributes outputAttributes,
              Monitor *monitor) {
  ...
  if (options.chunkMethod == "LASZIP") {
    ...
    chunker_countsort_laszip::doChunking(sources, targetDir, stats.min,
                                         stats.max, state, outputAttributes,
                                         monitor);
...
void createReport(Options &options, vector<Source> sources, string targetDir,
                  Stats &stats, State &state, double tStart) {
  double duration = now() - tStart;
  double throughputMB = (stats.totalBytes / duration) / (1024 * 1024);
  double throughputP =
      (double(stats.totalPoints) / double(duration)) / 1'000'000.0;
  ...
  cout << "input file size:       " << formatNumber(inputSize, 1)
       << inputSizeUnit << endl;
  cout << "duration:              " << formatNumber(duration, 3) << "s" << endl;
  cout << "throughput (MB/s)      " << formatNumber(throughputMB) << "MB"
       << endl;
  cout << "throughput (points/s)  " << formatNumber(throughputP, 1) << "M"
       << endl;
...
void generatePage(string exePath, string pagedir, string pagename) {
  ...
  { // configure page template
    ...
    string strPointcloud =
        stringReplace(strPointcloudTemplate, "<!-- URL -->", url);
    strPointcloud = stringReplace(strPointcloud, "<!-- NAME -->", pagename);

    string strPage = stringReplace(strTemplate, "<!-- INCLUDE POINTCLOUD -->",
                                   strPointcloud);
...
int main(int argc, char **argv) {
  ...
  // 	string hierarchyDir =
  // "D:/dev/pointclouds/Riegl/retz_converted/.hierarchyChunks"; 	int
  // hierarchyStepSize = 4;
  ...
  if (exePath.find("build") != std::string::npos) {
    exePath = fs::canonical(fs::absolute(argv[0])).parent_path().parent_path().string();
  }
#endif
